...
try:
    CKPT_PATH = 'model_best_epoch96_edit.pth'
...
if __name__ == '__main__':
    ...
    img_tensor = img_tensor.view((1, 3, 1024, 512))
    # print(img_tensor.shape)
    img_tensor = img_tensor.float()
    in_ = img_tensor.to(device)
    # in_ = torch.ones(1, 3, 768, 768).to(device)
    y = model(in_)
    # y = y.detach().cpu().numpy()
    # y = y.reshape(480, 128, 64)
    # img = Image.fromarray(y, 'RGB')
    # img.save('demo.png')
    # img.show()
    print(y.shape)
    # preds = y.max(1)[1].detach().cpu().numpy()
    # print(preds)
